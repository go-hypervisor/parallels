...
/* Creates a packed problem report and returns a handle of type PHT_PROBLEM_REPORT,
...
PRL_METHOD_DECL( PARALLELS_API_VER_8,
             PrlApi_CreatePackedProblemReport, (
...
/* Obtains a client version on the problem report.
   Parameters
   hProblemReport       : [in] A handle of type PHT_PROBLEM_REPORT
                          identifying the problem report object.
   sClientVersion           : [out] A pointer to a buffer that receives
                          the result (a UTF\-8 encoded,
                          null\-terminated string).
   pnBufLength : [in] The size of the output buffer (in
                          bytes). Set the buffer pointer to null
...
/* Sets a client version on the problem report.
   ...
   hProblemReport : [in] A handle of type PHT_PROBLEM_REPORT
                    identifying the problem report object.
   sClientVersion: [in] The value to set - must be a a UTF\-8
                    encoded, null\-terminated string.
   Returns
   PRL_RESULT. Possible values:
...
/* Sets problem report's CombineReportId which is used to match
   two reports on a server side.

   Parameters
   hProblemReport : [in] A handle of type PHT_PROBLEM_REPORT.
   sReportType : [in] CombineReport id string
...
PRL_METHOD_DECL( PARALLELS_API_VER_5,
                 PrlReport_SetCombinedReportId, (
                    PRL_HANDLE hProblemReport,
                    PRL_CONST_STR sCombinedReportId
        ) );

/* Gets problem report's CombineReportId which is used to match several reports
   on a server side.

   Parameters
   hProblemReport : [in] A handle of type PHT_PROBLEM_REPORT.
   sReportType : [out] CombineReport id string pointer
   pnCombinedReportIdLength : [in out] pointer to a size of CombinedReportId string
...
/* Sets rating of product using.

   Parameters
   hProblemReport : [in] A handle of type PHT_PROBLEM_REPORT.
   nRating : [in] Rating value.
...
/* Gets rating of product using.
    ...
    Parameters
    hProblemReport : [in] A handle of type PHT_PROBLEM_REPORT.
    pnRating : [out] pointer to a rating variable.
...
/* The PrlApi_ScaleImage scales image and put a new image to a buffer.
   Parameters
   pOriginalImage :      [in] Original image raw data.
   nOriginalImageSize :  Original image data size.
   nFormat :             Image format to use.
   nQuality :            Image quality factor.
                         Must be in the range from 0 to 100, or \-1.
						 Specify 0 to use the highest data compression,
						 100 for no compression, and \-1 for default.
   nScaleWidth :         The width of the area to scale (pixels).
   nScaleHeight :        The height of the area to scale (pixels).
   pNewImage :           [out] New image raw data.
   pnNewImageSize :      New image data size.
                         [in] The size of the output buffer
                         (in bytes). Set the buffer pointer
                         to null and this parameter's value
                         to zero to receive the required size.
